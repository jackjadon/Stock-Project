...
stock_data[TEST_STOCK_CODE] = {KEYS[0]: [], KEYS[1]: False}
...
def check_rate_of_change(arr):
    ...
    for i in range(DATA_RANGE):
        ...
        #Maybe change this?
        if roc == 0 or abs((current_roc-roc)/roc) > 1:
            if(lenient):
                lenient = False
            else:
                stable_stock = False
...
def check_stock(code, price_history, stable):
    if(len(price_history) > DATA_RANGE + TIME_ELAPSED):
        ...
        stock_data[TEST_STOCK_CODE][KEYS[1]] = tup[1]
    else:
        print("N")
...
while True:
    ...
    #Tests code on a test stock, then ends the loop
    if len(sys.argv) > 1:
        print(sys.argv)
        f = open("data")
        for x in f:
            stock_data[TEST_STOCK_CODE][KEYS[0]].append(float(x))
            check_stock(TEST_STOCK_CODE, 
                            stock_data[TEST_STOCK_CODE][KEYS[0]], 
                            stock_data[TEST_STOCK_CODE][KEYS[1]])
        break
